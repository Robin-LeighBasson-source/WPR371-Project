...
// Ensure search modal is closed on page load
...
const revealEls = [
  ...document.querySelectorAll(
    ".tag, h1, h2, .body-text, .event-details, .cta, .h-line"
  )
];
...
const floatingHeader = document.getElementById("floating_header");
...
let isFullDetailsMode = false;

function openEventModal(eventId, title, description, date, category, fetchFull = false) {
  currentEventId = eventId;
  const modal = document.getElementById("event_modal");
  const modalContent = modal.querySelector(".modal-content");
  
  // Reset to slide-in state
  modalContent.classList.remove("full-details");
  modalContent.style.animation = "none";
  setTimeout(() => {
    modalContent.style.animation = "";
  }, 10);
  
  displayBasicEventDetails(title, description, date, category);
  modal.classList.add("open");
  document.body.style.overflow = "hidden";
}

function displayBasicEventDetails(title, description, date, category) {
  const modal = document.getElementById("event_modal");
  const modalContent = modal.querySelector(".modal-content");
  
  isFullDetailsMode = false;
  ...
  document.getElementById("modal_category_value").textContent = category;
  
  // Update button text
  document.getElementById("modal_action_btn").innerHTML = `
    View Details
...
function handleSearchSubmit() {
  // Modal will close and form will submit naturally
  closeSearchModal();
